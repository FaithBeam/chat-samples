...
class DatabaseConnection:
    ...
    def bottom(self, column: str, limit=3):
        return (
    ...
    def delete_record(self, column: str, value):
        self.session.delete(self.get_record(column, value))
        self.session.commit()

    def get_record(self, column: str, value):
        return (
            self.session.query(self.table_name)
    ...
    def top(self, column: str, limit=3):
        return (
            self.session.query(self.table_name)
    ...
    def update_record(self, column: str, value, set: dict):
        self.session.query(self.table_name).filter(
            getattr(self.table_name, column) == value
        ).update(set)
        self.session.commit()
